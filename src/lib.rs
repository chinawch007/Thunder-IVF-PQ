...
fn euclidean_distance(v1 : &Point, v2 : &Point) -> f64 {
    v1.iter().zip(v2.iter())
        .map(|(a, b)| (a - b).powi(2))
        .sum::<f64>()
        .sqrt()
    ...
    loop { //until convergence
        for v in vectors {
            ...
            for (i, centroid) in centroids.iter().enumerate() {
                let distance = euclidean_distance(&v, &centroid);
...
/*
fn k_means(vectors: &Vec<Vec<f64>>, k: usize, max_iterations: usize) -> Vec<Vec<f64>> {
    let mut centroids = get_centroids(vectors, k);
    ...
    for _ in 0..max_iterations {
        ...
        for vector in vectors {
            ...
            clusters[closest_centroid].push(vector.clone());
        }
